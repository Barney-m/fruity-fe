...
function ProductFilterDrawer({ filter, initialState, handleDrawerOpen, openFilterDrawer, setFilter, setLoading }: FilterDrawerProps) {
  ...
  const filterIsEqual = (a1: ProductsFilter, a2: ProductsFilter) =>
    ...
    (a1.length === a2.length &&
      ...
      a1.price === a2.price &&
      a1.rating === a2.rating &&
      JSON.stringify(a1.grade) === JSON.stringify(a2.grade) &&
  ...
  const handleFilter = (type: string, params: string, rating?: number) => {
    ...
    switch (type) {
      case 'grade':
        if (filter.grade.some((item) => item === params)) {
          setFilter({ ...filter, grade: filter.grade.filter((item) => item !== params) });
        } else {
          setFilter({ ...filter, grade: [...filter.grade, params] });
        }
        break;
      case 'categories':
