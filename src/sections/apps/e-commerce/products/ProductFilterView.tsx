...
const ProductFilterView = ({ filter, filterIsEqual, handleFilter, initialState }: ProductFilterViewProps) => {
    ...
    <>
      {!filterIsEqual(initialState, filter) && (
        <Stack spacing={2}>
          ...
          {!(JSON.stringify(initialState.grade) === JSON.stringify(filter.grade)) && (
            <Grid item>
              <Stack>
                <Typography variant="subtitle1">Grade</Typography>
                <Grid container item sx={{ ml: '-10px' }}>
                  {filter.grade.map((item: string, index: number) => (
                    <Stack direction="row" alignItems="center" key={index}>
                      ...
                      <IconButton
                        ...
                        size="small"
                        sx={{ '&:hover': { bgcolor: 'transparent' }, ml: -1.5 }}
                        onClick={() => handleFilter('grade', item)}
                      >
                        <CloseOutlined />
                      </IconButton>
                    </Stack>
